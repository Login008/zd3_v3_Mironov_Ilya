...
namespace OtdelLibrary
{
    public class Otdel
    {
        ...
        virtual public List<string[]> GetInfo() //Вывод информации в виде списка массивов
        {
            ...
            {
                string[] array = [item.name, item.baseOklad.ToString(), item.k.ToString(), item.members.ToString(),
                    item.nameOfCompany];
                list1.Add(array);
            }
        ...
        static public void AddOtdel(string name, double baseOklad, double k, int members, string nameOfCompany) 
            //Добавление отдела в список
        {
            Otdel otdel = new Otdel(name, baseOklad, k, members, nameOfCompany); //Создаём объект и вызываем
                                                                                 //другую перегрузку
            otdel.AddOtdel();
        }
        static public void AddOtdel(Otdel otdel) //Перегрузка добавления отдела в список
        {
            if (otdel.baseOklad > 0 && otdel.k >= 0 && otdel.k <= 2 && otdel.members > 0) //проверка на адекватность
                                                                                          //значений
            {
                var selectedOtdels = from t in list
                                     where t.name == otdel.name && t.baseOklad == otdel.baseOklad && t.k == otdel.k 
                                     && t.members == otdel.members && t.nameOfCompany == otdel.nameOfCompany
                                     select t; //с помощью linq-запроса проверяем, есть ли в списке отдел
                                               //с такими же данными
        ...
        {
            if (baseOklad > 0 && k >= 0 && k <= 2 && members > 0) //Всё то же самое, только добавляем в список отдел,
                                                                  //с помощью которого вызвали перегрузку метода
            {
                var selectedOtdels = from t in list
                                     where t.name == name && t.baseOklad == baseOklad && t.k == k 
                                     && t.members == members && t.nameOfCompany == nameOfCompany
                                     select t;
        ...
        static public void RemoveOtdel(string name, double baseOklad, double k, int members, string nameOfCompany) 
            //Удаление отдела из списка
        {
            Otdel otdel = new Otdel(name, baseOklad, k, members, nameOfCompany); //Создаём перегрузку и вызываем
                                                                                 //другую перегрузку
            RemoveOtdel(otdel);
        }
